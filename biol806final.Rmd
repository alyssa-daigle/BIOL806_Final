...
#   summarise(Samples = n())
# 
# sample_table <- kable(sample_numbers, format = "html", caption = "Number of Samples")
# 
# print(sample_table)
```
...
```{r}
#seasonality - need a DF that breaks dates into seasons (should have spring, summer, fall and winter, but spring/winter will only have one month each (May and December)).

#alyssa checking to see if this works
#ciana checking

# have date as a numeric value in some way that makes sense, linear model for each color, fit a quadratic line
# lm(percentBeige ~ time + I(time^2))
# lmer(percentBeige ~ time + timesquared + 1|site + timesquared|site)
...
```{r testing a second option for the first line plot, easier code}
library(ggplot2)
...
location_list <- c("Durham Reservoir", 
                   "LaRoche Pond", 
                   "Mill Pond", 
                   "Thompson Farm", 
                   "Upper Mill Pond",
...
mapshot(sampling_map, file = "sampling_locations.png")
